...
import { backendPulumiResourcesPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [backendPulumiResourcesPlugin],
    });

    await request(server).get('/api/backend-pulumi-resources/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/backend-pulumi-resources/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/backend-pulumi-resources/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/backend-pulumi-resources/todos/${createdTodoItem.id}`)
